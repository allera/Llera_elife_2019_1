...
%--> alb  
%save_vest(M.H', [outdir 'subjectCoursesOut.txt'])% original code
save_vest_pretty(M.H', [outdir 'subjectCoursesOut.txt'])% my substitution
%dlmwrite([outdir 'subjectCoursesOut.txt'],M.H'); % my older substitution
%<--alb
...
%--> alb added _pretty 
save_vest_pretty(tmp, [outdir 'noiseStdevOut.txt']);
save_vest_pretty(tmp.^-2, [outdir 'noisePrecisionOut.txt']); % A more useful plot to find extremely low-noise subjects (likely duplicate data!)

%dlmwrite([outdir 'noiseStdevOut.txt'],tmp); % my older substitutions
%dlmwrite([outdir 'noisePrecisionOut.txt'],tmp.^-2);
%<-- alb
...
dlmwrite([outdir 'convergenceRate.txt'],tmp'); % my substitution
loglog(tmp(1,:), tmp(3,:), '.-')
clear tmp*
