%alb commented paragraph at line 130 over missing subjects
...
%elseif iscell(transforms) && isequal(size(transforms), [1 4])     changed alb
elseif iscell(transforms) && isequal(size(transforms), [1 length(Yfiles)])
   
    transforms = repmat(transforms, length(Yfiles), 1);
else
    assert(isequal(size(transforms), [length(Yfiles) 4])) %  next line alb
    %assert(isequal(size(transforms), [length(Yfiles) length(Yfiles)]))
end
    ...
    elseif strfind(Yfiles{k},'.mgh')
        ...
        switch (NvoxPerHemi) 
            case 2562
                ...
                labelSrcDir = '/usr/local/freesurfer/subjects/fsaverage4/label/';
            case 10242
                labelSrcDir = '/vols/Data/oslo/fsaverages/fsaverage5/label/';
                labelSrcDir = '/usr/local/freesurfer/subjects/fsaverage5/label/';
            case 40962
                labelSrcDir = '/vols/Data/oslo/fsaverages/fsaverage6/label/';
                labelSrcDir = '/usr/local/freesurfer/subjects/fsaverage6/label/';
            case 163842
                labelSrcDir = '/vols/Data/oslo/fsaverages/fsaverage/label/';
                labelSrcDir = '/usr/local/freesurfer/subjects/fsaverage/label/';
                labelSrcDir = '/Applications/freesurfer/subjects/fsaverage/label/' %'/vols/Data/oslo/fsaverages/fsaverage_/label/';
            otherwise
                labelSrcDir = '';
                size(vol,2)
